...
--
--  Patrones aplicados (los mismos que la version Pentaho/SQL, ya validados):
--    * Dimensiones : SCD Tipo 1. Se recorre la fuente FILA POR FILA con un
--                    CURSOR (FOR ... IN SELECT ... LOOP); si la clave natural
--                    ya existe se hace UPDATE, si no, se calcula la clave
--                    subrogada (SK = maximo actual + 1) y se hace INSERT.
--                    Re-ejecutable sin duplicar.
--    * DIM_TIEMPO  : GENERADA, no extraida. Bucle WHILE que avanza dia a dia
--                    del 2018-01-01 al 2031-12-31 calculando cada atributo.
--    * Hechos      : full refresh (TRUNCATE + carga). Por cada registro de la
--                    fuente se resuelven las SK con lookups (SELECT ... INTO)
--                    y se inserta el hecho.
--    * Miembro por defecto -1 "NO APLICA" (patron Kimball) para las FK de los
--                    hechos que no aplican al grano (FACT_METAS).
--
--  Como ejecutarlo:
--    1) Requiere la fuente y el DW ya creados (puntos 1 y 2) y el
...
--       Las trazas (RAISE NOTICE) muestran el avance paso a paso.
--
...
DECLARE
    ...
    v_fecha     DATE;
    v_sk        INTEGER;
    v_dow       INTEGER;   -- 0=Domingo..6=Sabado (EXTRACT DOW) -> +1 para el proyecto
    v_mes       INTEGER;
    v_insertadas INTEGER := 0;
    -- Catalogos de nombres en espanol (indice base 1)
...
BEGIN
    ...
    WHILE v_fecha <= c_fecha_fin LOOP
        ...
        IF NOT EXISTS (SELECT 1 FROM seguro_dw_g30871276.dim_tiempo t
            ...
            VALUES
                  (v_sk,
                   ...
                   v_mes,
                   EXTRACT(DOY  FROM v_fecha)::int,
                   EXTRACT(DAY  FROM v_fecha)::int,
                   v_dow + 1,                                   -- 1=Domingo..7=Sabado
                   EXTRACT(WEEK FROM v_fecha)::int,
                   c_dias[v_dow + 1],
                   c_dias_cor[v_dow + 1],
                   c_meses[v_mes],
                   c_meses_cor[v_mes],
        ...
        v_fecha := v_fecha + 1;   -- avanzar un dia
    END LOOP;
...
-- ============================================================================
--  2. DIM_CLIENTE  (SCD1 por cod_cliente_nk, recorrido con cursor)
-- ============================================================================
...
BEGIN
    ...
    LOOP
        IF EXISTS (SELECT 1 FROM seguro_dw_g30871276.dim_cliente
                   WHERE cod_cliente_nk = r.cod_cliente) THEN
            -- Ya existe: SCD1 -> sobreescribir atributos, conservar SK
        ...
        ELSE
            -- Nuevo: asignar SK y dar de alta
            v_sk := v_sk + 1;
    ...
    -- Miembro por defecto -1 "NO APLICA" (usado por FACT_METAS)
...
-- ============================================================================
--  3. DIM_PRODUCTO  (desnormaliza PRODUCTO + TIPO_PRODUCTO; SCD1 por cod_producto_nk)
-- ============================================================================
...
BEGIN
    ...
    LOOP
        IF EXISTS (SELECT 1 FROM seguro_dw_g30871276.dim_producto
                   WHERE cod_producto_nk = r.cod_producto) THEN
...
BEGIN
    ...
    LOOP
        IF EXISTS (SELECT 1 FROM seguro_dw_g30871276.dim_contrato
                   WHERE nro_contrato_nk = r.nro_contrato) THEN
            UPDATE seguro_dw_g30871276.dim_contrato
               SET descrip_contrato = r.descrip_contrato
             WHERE nro_contrato_nk = r.nro_contrato;
            v_cambios := v_cambios + 1;
        ELSE
            v_sk := v_sk + 1;
            INSERT INTO seguro_dw_g30871276.dim_contrato
                  (sk_dim_contrato, nro_contrato_nk, descrip_contrato)
...
-- ============================================================================
--  5. DIM_ESTADO_CONTRATO  (dominio fijo: dominio del CHECK de la fuente)
-- ============================================================================
...
BEGIN
    ...
    SELECT COALESCE(MAX(sk_dim_estado_contrato), 0) INTO v_sk
    FROM seguro_dw_g30871276.dim_estado_contrato;

    FOR r IN SELECT * FROM (VALUES
                ('ACTIVO','Activo'), ('VENCIDO','Vencido'), ('SUSPENDIDO','Suspendido')
             ) AS v(cod_estado, descrip_estado)
    LOOP
        IF NOT EXISTS (SELECT 1 FROM seguro_dw_g30871276.dim_estado_contrato
                       WHERE cod_estado = r.cod_estado) THEN
            v_sk := v_sk + 1;
            INSERT INTO seguro_dw_g30871276.dim_estado_contrato
                  (sk_dim_estado_contrato, cod_estado, descrip_estado)
...
BEGIN
    ...
    SELECT COALESCE(MAX(sk_dim_evaluacion), 0) INTO v_sk
    FROM seguro_dw_g30871276.dim_evaluacion_servicio;
    ...
    LOOP
        IF EXISTS (SELECT 1 FROM seguro_dw_g30871276.dim_evaluacion_servicio
                   WHERE cod_evaluacion_nk = r.cod_evaluacion_servicio) THEN
            UPDATE seguro_dw_g30871276.dim_evaluacion_servicio
               SET nb_descrip = r.nb_descripcion
             WHERE cod_evaluacion_nk = r.cod_evaluacion_servicio;
            v_cambios := v_cambios + 1;
        ELSE
            v_sk := v_sk + 1;
            INSERT INTO seguro_dw_g30871276.dim_evaluacion_servicio
                  (sk_dim_evaluacion, cod_evaluacion_nk, nb_descrip)
...
-- ============================================================================
--  7. DIM_SUCURSAL  (desnormaliza SUCURSAL + CIUDAD + PAIS; SCD1 por cod_sucursal_nk)
-- ============================================================================
...
BEGIN
    ...
    SELECT COALESCE(MAX(sk_dim_sucursal), 0) INTO v_sk
    FROM seguro_dw_g30871276.dim_sucursal;

    FOR r IN SELECT su.cod_sucursal, su.nb_sucursal, ci.cod_ciudad, ci.nb_ciudad,
                    pa.cod_pais, pa.nb_pais
    ...
    LOOP
        IF EXISTS (SELECT 1 FROM seguro_dw_g30871276.dim_sucursal
                   WHERE cod_sucursal_nk = r.cod_sucursal) THEN
...
BEGIN
    ...
    SELECT COALESCE(MAX(sk_dim_siniestro), 0) INTO v_sk
    FROM seguro_dw_g30871276.dim_siniestro;

    FOR r IN SELECT nro_siniestro, descripcion_siniestro
             FROM seguro_g30871276.siniestro ORDER BY nro_siniestro
    LOOP
        IF EXISTS (SELECT 1 FROM seguro_dw_g30871276.dim_siniestro
                   WHERE nro_siniestro_nk = r.nro_siniestro) THEN
            UPDATE seguro_dw_g30871276.dim_siniestro
               SET descrip_siniestro = r.descripcion_siniestro
             WHERE nro_siniestro_nk = r.nro_siniestro;
            v_cambios := v_cambios + 1;
        ELSE
            v_sk := v_sk + 1;
            INSERT INTO seguro_dw_g30871276.dim_siniestro
                  (sk_dim_siniestro, nro_siniestro_nk, descrip_siniestro)
...
-- ============================================================================
--  9. FACT_REGISTRO_CONTRATO  (full refresh; lookups fila por fila)
--     Sucursal del hecho = sucursal de captacion del cliente.
--     Contadores a 1 (los conteos distintos se resuelven en la capa OLAP).
-- ============================================================================
...
DECLARE
    ...
    v_filas    INTEGER := 0;
BEGIN
    RAISE NOTICE '-> FACT_REGISTRO_CONTRATO (full refresh)';
    TRUNCATE seguro_dw_g30871276.fact_registro_contrato;

    FOR r IN SELECT rc.nro_contrato, rc.cod_producto, rc.cod_cliente,
                    rc.fecha_inicio, rc.fecha_fin, rc.monto, rc.estado_contrato,
                    cl.cod_sucursal
             FROM seguro_g30871276.registro_contrato rc
             JOIN seguro_g30871276.cliente cl ON cl.cod_cliente = rc.cod_cliente
    LOOP
        -- Resolucion de claves subrogadas (lookups por clave natural)
        SELECT sk_dim_tiempo INTO v_sk_ini FROM seguro_dw_g30871276.dim_tiempo
               WHERE fecha_completa = r.fecha_inicio;
        SELECT sk_dim_tiempo INTO v_sk_fin FROM seguro_dw_g30871276.dim_tiempo
               WHERE fecha_completa = r.fecha_fin;
        SELECT sk_dim_cliente INTO v_sk_cli FROM seguro_dw_g30871276.dim_cliente
               WHERE cod_cliente_nk = r.cod_cliente;
        SELECT sk_dim_contrato INTO v_sk_con FROM seguro_dw_g30871276.dim_contrato
               WHERE nro_contrato_nk = r.nro_contrato;
        SELECT sk_dim_producto INTO v_sk_pro FROM seguro_dw_g30871276.dim_producto
               WHERE cod_producto_nk = r.cod_producto;
        SELECT sk_dim_estado_contrato INTO v_sk_est FROM seguro_dw_g30871276.dim_estado_contrato
               WHERE cod_estado = UPPER(r.estado_contrato);
        SELECT sk_dim_sucursal INTO v_sk_suc FROM seguro_dw_g30871276.dim_sucursal
               WHERE cod_sucursal_nk = r.cod_sucursal;
        ...
        VALUES (v_sk_ini, v_sk_fin, v_sk_cli, v_sk_con, v_sk_pro, v_sk_est, v_sk_suc,
                r.monto, 1, 1, 1, 1);

        v_filas := v_filas + 1;
    END LOOP;

    RAISE NOTICE '   FACT_REGISTRO_CONTRATO: % filas.', v_filas;
END;
$$;


-- ============================================================================
--  10. FACT_REGISTRO_SINIESTRO  (full refresh)
...
DECLARE
    ...
    v_sk_tsin  INTEGER;
    v_filas    INTEGER := 0;
    v_sin_resp INTEGER := 0;
BEGIN
    RAISE NOTICE '-> FACT_REGISTRO_SINIESTRO (full refresh)';
    TRUNCATE seguro_dw_g30871276.fact_registro_siniestro;
    ...
    FOR r IN SELECT rs.nro_siniestro, rs.nro_contrato, rs.fecha_siniestro, rs.fecha_respuesta,
             ...
             JOIN seguro_g30871276.cliente cl ON cl.cod_cliente = rc.cod_cliente
    LOOP
        SELECT sk_dim_tiempo INTO v_sk_sin FROM seguro_dw_g30871276.dim_tiempo
               WHERE fecha_completa = r.fecha_siniestro;

        -- Fecha de respuesta puede no existir: se maneja de forma explicita
        IF r.fecha_respuesta IS NULL THEN
            v_sk_resp := NULL;
            v_sin_resp := v_sin_resp + 1;
        ELSE
            SELECT sk_dim_tiempo INTO v_sk_resp FROM seguro_dw_g30871276.dim_tiempo
                   WHERE fecha_completa = r.fecha_respuesta;
        END IF;
        ...
        SELECT sk_dim_producto INTO v_sk_pro  FROM seguro_dw_g30871276.dim_producto
               WHERE cod_producto_nk = r.cod_producto;
        SELECT sk_dim_siniestro INTO v_sk_tsin FROM seguro_dw_g30871276.dim_siniestro
               WHERE nro_siniestro_nk = r.nro_siniestro;
    ...
    RAISE NOTICE '   FACT_REGISTRO_SINIESTRO: % filas (% sin fecha de respuesta).', v_filas, v_sin_resp;
END;
$$;


-- ============================================================================
--  11. FACT_EVALUACION_SERVICIO  (full refresh)
--      Regla de negocio: la evaluacion ocurre al finalizar el contrato ->
...
--      recomienda_amigo: SI -> 1.00, NO -> 0.00 (AVG() da el % de los indic. 13/14).
-- ============================================================================
...
BEGIN
    RAISE NOTICE '-> FACT_EVALUACION_SERVICIO (full refresh)';
    TRUNCATE seguro_dw_g30871276.fact_evaluacion_servicio;

    FOR r IN SELECT cod_cliente, cod_producto, cod_evaluacion_servicio, recomienda_amigo
             FROM seguro_g30871276.recomienda
    LOOP
        -- Fecha de evaluacion derivada: ultimo fin de contrato del par (cliente, producto)
        SELECT MAX(rc.fecha_fin) INTO v_fecha_eval
        FROM   seguro_g30871276.registro_contrato rc
        WHERE  rc.cod_cliente = r.cod_cliente AND rc.cod_producto = r.cod_producto;

        -- Filtro de calidad: sin contrato -> evaluacion huerfana -> se descarta
        IF v_fecha_eval IS NULL THEN
            v_descartadas := v_descartadas + 1;
            CONTINUE;
        END IF;

        SELECT sk_dim_tiempo INTO v_sk_tiempo FROM seguro_dw_g30871276.dim_tiempo
               WHERE fecha_completa = v_fecha_eval;
        SELECT sk_dim_cliente INTO v_sk_cli FROM seguro_dw_g30871276.dim_cliente
               WHERE cod_cliente_nk = r.cod_cliente;
        SELECT sk_dim_producto INTO v_sk_pro FROM seguro_dw_g30871276.dim_producto
               WHERE cod_producto_nk = r.cod_producto;
        SELECT sk_dim_evaluacion INTO v_sk_eval FROM seguro_dw_g30871276.dim_evaluacion_servicio
               WHERE cod_evaluacion_nk = r.cod_evaluacion_servicio;

        -- Transformacion SI/NO -> 1.00/0.00
    ...
    RAISE NOTICE '   FACT_EVALUACION_SERVICIO: % filas (% descartadas por filtro de calidad).',
                 v_filas, v_descartadas;
END;
$$;


-- ============================================================================
--  12. FACT_METAS  (full refresh)
--      Fuente real: hoja Excel de la Gerencia de Estadistica (Metas_Seguros.xlsx).
--      Aqui las 50 metas (2022-2026, 10 productos) se cargan embebidas para que
--      el ETL procedimental sea autocontenido y equivalente al flujo Pentaho.
--      Grano: meta anual por producto -> cliente y contrato usan el miembro -1.
-- ============================================================================
...
DECLARE
    ...
    v_filas   INTEGER := 0;
BEGIN
    RAISE NOTICE '-> FACT_METAS (full refresh, origen: hoja de metas)';
    TRUNCATE seguro_dw_g30871276.fact_metas;

    FOR r IN SELECT * FROM (VALUES
            -- (anio, cod_producto, monto_meta_ingreso, meta_renovacion, meta_asegurados)
            (2022, 1, 260.00, 1, 2),  (2022, 2, 960.00, 1, 1),  (2022, 3, 585.00, 1, 2),
            (2022, 4, 144.00, 1, 1),  (2022, 5, 390.00, 1, 2),  (2022, 6, 2000.00, 1, 1),
            (2022, 7, 195.00, 1, 2),  (2022, 8, 640.00, 1, 1),  (2022, 9, 780.00, 1, 2),
            (2022, 10, 96.00, 1, 1),
            (2023, 1, 160.00, 1, 1),  (2023, 2, 1560.00, 1, 2), (2023, 3, 360.00, 1, 1),
            (2023, 4, 234.00, 1, 2),  (2023, 5, 240.00, 1, 1),  (2023, 6, 3250.00, 1, 2),
            (2023, 7, 120.00, 1, 1),  (2023, 8, 1040.00, 1, 2), (2023, 9, 480.00, 1, 1),
            (2023, 10, 156.00, 1, 2),
            (2024, 1, 260.00, 1, 2),  (2024, 2, 960.00, 1, 1),  (2024, 3, 585.00, 1, 2),
            (2024, 4, 144.00, 1, 1),  (2024, 5, 390.00, 1, 2),  (2024, 6, 2000.00, 1, 1),
            (2024, 7, 195.00, 1, 2),  (2024, 8, 640.00, 1, 1),  (2024, 9, 780.00, 1, 2),
            (2024, 10, 96.00, 1, 1),
            (2025, 1, 160.00, 1, 1),  (2025, 2, 1560.00, 1, 2), (2025, 3, 360.00, 1, 1),
            (2025, 4, 234.00, 1, 2),  (2025, 5, 240.00, 1, 1),  (2025, 6, 3250.00, 1, 2),
            (2025, 7, 120.00, 1, 1),  (2025, 8, 1040.00, 1, 2), (2025, 9, 480.00, 1, 1),
            (2025, 10, 156.00, 1, 2),
            (2026, 1, 260.00, 1, 2),  (2026, 2, 960.00, 1, 1),  (2026, 3, 585.00, 1, 2),
            (2026, 4, 144.00, 1, 1),  (2026, 5, 390.00, 1, 2),  (2026, 6, 2000.00, 1, 1),
            (2026, 7, 195.00, 1, 2),  (2026, 8, 640.00, 1, 1),  (2026, 9, 780.00, 1, 2),
            (2026, 10, 96.00, 1, 1)
         ) AS m(anio, cod_producto, monto_meta_ingreso, meta_renovacion, meta_asegurados)
    LOOP
        -- Periodo de la meta: 1 de enero -> 31 de diciembre del anio
        SELECT sk_dim_tiempo INTO v_sk_ini FROM seguro_dw_g30871276.dim_tiempo
               WHERE fecha_completa = MAKE_DATE(r.anio, 1, 1);
        SELECT sk_dim_tiempo INTO v_sk_fin FROM seguro_dw_g30871276.dim_tiempo
               WHERE fecha_completa = MAKE_DATE(r.anio, 12, 31);
        SELECT sk_dim_producto INTO v_sk_pro FROM seguro_dw_g30871276.dim_producto
               WHERE cod_producto_nk = r.cod_producto;
        ...
        VALUES (v_sk_ini, v_sk_fin, -1, v_sk_pro, -1,
                r.monto_meta_ingreso, r.meta_renovacion, r.meta_asegurados);

        v_filas := v_filas + 1;
    END LOOP;

    RAISE NOTICE '   FACT_METAS: % filas.', v_filas;
END;
$$;
...
--  Ejecuta las dimensiones primero y los hechos despues, con manejo de error.
-- ============================================================================
...
BEGIN
    ...
    -- 1) Dimensiones (deben ir siempre antes que los hechos)
...
EXCEPTION
    WHEN OTHERS THEN
        -- Cualquier error aborta la carga completa (rollback) y se reporta con contexto
